...
The baseline misses **29 of 200 sessions** (Hit Rate@10 = **85.50%**). This first-pass diagnosis uses evaluator outcomes plus per-turn retrieval/ranking traces; primary labels are mutually exclusive so counts reconcile exactly to the miss total.
...
- Rerank failures account for 25/29 misses; 12 are near-cutoff targets with a best merged-pool rank of 11-20.
- Override failures account for 3/29 misses and should be isolated before broad rank-weight tuning.
- Only 1/29 misses never retrieve the target, so adding recall routes is not the first global priority.
...
| intent_override | Override failure | 3 |
...
|---|---|---|---|---:|---:|---:|---:|
| public_0002 | intent_override | Rerank failure | Override context | 10 | 52 | 66 | 3 |
...
- Result source: `analysis/runs/integrated_guarded_safe_4e9be3c_cleanclone/result.json`
- Trace source: `analysis/runs/integrated_guarded_safe_4e9be3c_cleanclone/diagnostic_traces.jsonl`
- Trace coverage for misses: 29/29.
